...
!=======================================================================
! PICO_60_2017 ANALYSIS ROUTINES
! Based upon https://arxiv.org/pdf/1702.07666.pdf .  
!=======================================================================
...
!-----------------------------------------------------------------------
! Initializes a DetectorStruct to the PICO_60_2017 analysis.
! 
! This analysis includes only fluorine!
! The threshold efficiency is taken from https://arxiv.org/pdf/1503.00008.pdf
! and is identical to the one assumed for the PICO_2L analysis
!
FUNCTION PICO_60_2017_Init(intervals) RESULT(D)
  ...
  ! The runtime is reduced to include the detection efficiency of 85.1%.
  ! NOTE: PICO_60_2017 does not attempt background subtraction

  CALL SetDetector(D,mass=36.95d0,time=25.53d0,Nevents_tot=0,           &
                   Backgr_tot=0.0d0,Nelem=1,Zelem=(/9/),                &
                   NE=NE,E=E,Nbins=NBINS,eff_all=EFF,                   &
                   intervals=intervals,Emin=EMIN)
  D%eff_file = '[PICO_60_2017]'
  
END FUNCTION


! C++ interface wrapper
INTEGER(KIND=C_INT) FUNCTION C_PICO_60_2017_Init(intervals) &
 BIND(C,NAME='C_DDCalc_pico_60_2017_init') 
  USE ISO_C_BINDING, only: C_BOOL, C_INT
  IMPLICIT NONE
  LOGICAL(KIND=C_BOOL), INTENT(IN) :: intervals
  N_Detectors = N_Detectors + 1
  ALLOCATE(Detectors(N_Detectors)%p)
  Detectors(N_Detectors)%p = PICO_60_2017_Init(LOGICAL(intervals))
  C_PICO_60_2017_Init = N_Detectors
END FUNCTION
