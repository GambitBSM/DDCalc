...
FUNCTION PICO_60_Init(intervals) RESULT(D)
  ...
  EFF(3,:,0:) = RESHAPE( (/ EFF_I(:) /), SHAPE(EFF(3,:,0:)) )

  ! The fiducial mass is reduced to account for cut effiencies and furthermore divided by a trial factor of 1.8.

  CALL SetDetector(D,mass=7.99d0,time=92.8d0,Nevents_tot=0,             &
