PROGRAM DDLikelihood
  ...
  DO mDMi = 0,mDMsteps
    ...
    DO sigmaSIi = 0,sigmaSIsteps
      ...
      CALL DDCalc_CalcRates(Detector, WIMP, Halo)
      lnp = LogPoissonP(Detector%Nevents(0),Detector%MuSignal(0) + Detector%Backgr(0))
      WRITE (*,*) mDM,sigmaSI,DDCalc_LogLikelihood(Detector),-2*(DDCalc_LogLikelihood(Detector)-BGlogL), lnp
    END DO
  END DO
